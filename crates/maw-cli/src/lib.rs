...
use maw_auth::{
    ...
    redact_pair_code, reject_consent_plan, request_consent_plan, resolve_from_address,
...
fn run_auth_plan(argv: &[String]) -> CliOutput {
    ...
    match action {
        ...
        AuthPlanAction::HashBody { plan_json, body } => {
            let body_hash = hash_body(body.as_deref().map(str::as_bytes));
            CliOutput {
                code: 0,
                stdout: if plan_json {
...
fn parse_auth_plan_args(argv: &[String]) -> Result<AuthPlanAction, String> {
    let Some(kind) = argv.first().map(String::as_str) else {
        return Err(
            "auth: expected sign-v3, verify-request, loopback, from-address, or hash-body"
                .to_owned(),
        );
    };
    match kind {
