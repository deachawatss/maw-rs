...
case "$1" in
  list-windows)
    printf '13-nova|||0|||nova-oracle|||1|||%s
...
' ;;
      *)
        ...
        count=$((count + 1))
        printf '%s' "$count" > "$DONE_CAPTURE_COUNT"
...
fn psi_rescue_copies_uncommitted_files_without_overwrite() {
    ...
    let rescued = done_rescue_psi(&worktree, &main).expect("rescue ok");
    assert_eq!(
        std::fs::read_to_string(main.join("ψ/test.md")).expect("main file"),
        "main copy"
    );
    assert_eq!(rescued.len(), 1, "rescued paths: {rescued:?}");
    let rescued_path = &rescued[0];
    assert_eq!(rescued_path.parent(), Some(main.join("ψ").as_path()));
    assert!(
        rescued_path
            .file_name()
        ...
        "rescued path: {rescued_path:?}"
    );
    assert_eq!(
        std::fs::read_to_string(rescued_path).expect("rescued file"),
        "worktree copy"
    );
