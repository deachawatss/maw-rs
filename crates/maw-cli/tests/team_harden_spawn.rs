...
use maw_cli::{
    run_cli, team_caller_pane, team_omx_auto_kickoff_with, team_orphan_sweep_from_pids,
    team_spawn_pane_target,
};
...
#[test]
fn caller_pane_anchor_uses_tmux_pane_env() {
    let _guard = ENV_LOCK.lock().expect("env lock");
    let old = std::env::var_os("TMUX_PANE");
    std::env::set_var("TMUX_PANE", "%42");

    assert_eq!(team_caller_pane().as_deref(), Some("%42"));
    assert_eq!(team_spawn_pane_target(team_caller_pane()), "%42");
...
#[test]
fn omx_auto_kickoff_reads_prompt_and_sends() {
    ...
    team_omx_auto_kickoff_with(&mut runner, "%9", &prompt_path).expect("kickoff");

    assert!(runner
        .calls
        .iter()
        .any(|(command, args)| { command == "send-keys" && args.iter().any(|arg| arg == prompt) }));
...
#[test]
fn orphan_pane_sweep_detects_dead_pids() {
    let member_panes = vec!["%1".to_owned(), "%2".to_owned()];
    let pane_pids = vec![("%1".to_owned(), 101), ("%2".to_owned(), 202)];

    let zombies = team_orphan_sweep_from_pids(&member_panes, &pane_pids, |pid| pid == 101);

    assert_eq!(zombies, vec!["%2".to_owned()]);
}

#[test]
fn team_status_shows_zombie_count_when_orphan_sweep_finds_dead_pid() {
    let _guard = ENV_LOCK.lock().expect("env lock");
    ...
    write_file(
        &config,
        &format!(
            "{{\"name\":\"alpha\",\"members\":[{{\"name\":\"live\",\"tmuxPaneId\":\"%1\"}},{{\"name\":\"dead\",\"tmuxPaneId\":\"%2\"}}],\"createdAt\":0,\"callerPane\":\"%9\"}}"
        ),
    );
