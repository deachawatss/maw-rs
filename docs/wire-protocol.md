...
| `/api/send` | POST | `{target,text,attachments?,inbox?}` | success `{ok:true,target,text,source,lastLine?,state,receipt?,inbox?,warning?,reason?,wokeFor?}`; errors 404/409/500/502 with `{error,...}`. Rust's HTTP federation path refuses a pane-less multi-agent window with the structured 409 shape above and never reports delivered when post-send capture confirmation fails. | protected write. Source: `src/api/sessions.ts:356-359`, `src/api/sessions.ts:470-528`, `src/api/sessions.ts:531-577`, `src/api/sessions.ts:580-609`, `src/lib/elysia-auth.ts:22-35`; Rust: `crates/maw-cli/src/core_impl/serve.rs`. |
