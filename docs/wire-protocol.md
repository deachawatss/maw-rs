...
| `/api/capture` | GET | query `target` | `{content}` or `{content:"", error, target?, validWindows?, hint?}`. Rust resolves every pane-less window to an explicit pane only when it has exactly one recognized agent pane or exactly one total pane. It otherwise returns HTTP 409 `{ok:false,error:"pane_target_ambiguous"|"pane_target_not_found",target,resolvedTarget,candidates,state:"failed"}` rather than reading the focused pane. | public/read. Source: `src/api/sessions.ts:299-339`; Rust: `crates/maw-cli/src/core_impl/serve.rs`. |
...
| `/api/send` | POST | `{target,text,attachments?,inbox?}` | success `{ok:true,target,text,source,lastLine?,state,receipt?,inbox?,warning?,reason?,wokeFor?}`; errors 404/409/500/502 with `{error,...}`. Rust uses the same explicit-pane-or-409 rule as `/api/capture`; it never sends a pane-less target to the focused pane. A completed signed delivery is recorded through the existing locked atomic JSONL state path for its 24-hour idempotency TTL, so a server restart still deduplicates a valid retry. If post-send capture classification fails, or the durable idempotency record cannot be written, it returns delivered with a `warning` (never a 5xx) because injection already succeeded; a persistence-failure warning notes that a restart may allow a duplicate. | protected write. Source: `src/api/sessions.ts:356-359`, `src/api/sessions.ts:470-528`, `src/api/sessions.ts:531-577`, `src/api/sessions.ts:580-609`, `src/lib/elysia-auth.ts:22-35`; Rust: `crates/maw-cli/src/core_impl/serve.rs`. |
