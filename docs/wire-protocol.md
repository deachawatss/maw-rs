...
| `/api/capture` | GET | query `target` | `{content}` or `{content:"", error, target?, validWindows?, hint?}`. Rust returns HTTP 409 `{ok:false,error:"pane_target_ambiguous",target,resolvedTarget,candidates,state:"failed"}` for a pane-less multi-agent window; one agent pane becomes its explicit target. | public/read. Source: `src/api/sessions.ts:299-339`; Rust: `crates/maw-cli/src/core_impl/serve.rs`. |
...
| `/api/send` | POST | `{target,text,attachments?,inbox?}` | success `{ok:true,target,text,source,lastLine?,state,receipt?,inbox?,warning?,reason?,wokeFor?}`; errors 404/409/500/502 with `{error,...}`. Rust's HTTP federation path refuses a pane-less multi-agent window with the structured 409 shape above. If post-send capture classification fails, it returns delivered with a `warning` because injection succeeded but confirmation is unavailable. | protected write. Source: `src/api/sessions.ts:356-359`, `src/api/sessions.ts:470-528`, `src/api/sessions.ts:531-577`, `src/api/sessions.ts:580-609`, `src/lib/elysia-auth.ts:22-35`; Rust: `crates/maw-cli/src/core_impl/serve.rs`. |
