...
| `cross-team-queue` | ship-wasm | `--json`, optional `--recipient <name>` | No creds; no host capabilities in slice 1. | Install/invoke smoke plus pin check (`crates/maw-cli/tests/fleet_plugins_pin_check.rs`). | None for scaffold artifact; read-only inbox scanning waits for vault-root capability design. |
